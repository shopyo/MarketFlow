...
from userapi.security import get_safe_redirect
...
from modules.box__ecommerce.shop.helpers import clear_cart_data
...
class ProductSchema(ma.Schema):
    class Meta:
        model = Product
        fields = ("id", "barcode", "name", "selling_price", "product_store", "resources")

    product_store = ma.Nested(StoreSchema)
...
def homepage():
    ...
    featured_stores = Store.query.filter(Store.is_featured == True).all()
...
# @module_blueprint.route("/c/<category_name>/page/<int:page>")
# @module_blueprint.route("/c/<category_name>")
# def category_by_price(category_name, page=1):
#     pass
...
def category(category_name, page=1):
    ...
    products = []
    for subc in current_category.subcategories:
        for p in subc.products:
            if all(
                (
                    p.status == 'approved',
                    min_price is None or p.selling_price >= min_price,
                    max_price is None or p.selling_price <= max_price
                )
            ):
                products.append(p)
...
@module_blueprint.route("api/sub/<subcategory_name>/page/<int:page>")
@module_blueprint.route("api/sub/<subcategory_name>")
def get_subcategory_products(subcategory_name, page=1):
    PAGINATION = 9
    end = page * PAGINATION
    start = end - PAGINATION
    ...
    if subcategory is None:
        return jsonify({"status": "failure", "result": []})
    ...
    for product in subcategory.products:
        if all(
            (
                ...
                max_price is None or product.selling_price <= max_price
            )
        ):
            products.append(product)

    total_pages = math.ceil(len(products) / PAGINATION)
    products = products[start:end]
...
@module_blueprint.route("/product/<product_id>")
...
@module_blueprint.route("/cart/add/<product_id>", methods=["GET", "POST"])
def cart_add(product_id):
    gotourl = request.args.get('next')
    if request.method == "POST":
        flash("")
        if "cart" in session:

            barcode = request.form["product_id"]
            ...
            if barcode not in data:
                ...
                if updated_quantity > product.in_stock:
                    ...
                    return redirect(
                        url_for("shop.product", product_id=barcode)
                    )
    ...
    return redirect(get_safe_redirect(gotourl))


@module_blueprint.route(
    "/cart/remove/<product_id>", methods=["GET", "POST"]
)
...
def checkout():
    ...
    form.default_country.choices = [('mauritius', 'Mauritius')]
    form.diff_country.choices = [('mauritius', 'Mauritius')]

    locality_sub_choices = [['{} > {} > {}'.format(c.locality, c.sublocality, c.street), '{} > {} > {}'.format(c.locality, c.sublocality, c.street)] for c in CitySublocality.query.all()]
    form.default_town_city.choices = locality_sub_choices
    form.diff_town_city.choices = locality_sub_choices
...
def checkout_process():
    if request.method == "POST":
        ...
        form = CheckoutForm()
        # return form.default_town_city.data
        cart_info = get_cart_data()
        ...
        if form.validate_on_submit():
            ...
            shipment_option = DeliveryOption.query.get(
                request.form["deliveryoption"]
            )
            order.shipment_option = shipment_option
            ...
            # changed to id, still named barcode
            for barcode in cart_data:
                order_item = OrderItem()
                product = Product.query.get(barcode)
                order_item.order_item_product = product
                order_item.order_item_store = product.product_store
